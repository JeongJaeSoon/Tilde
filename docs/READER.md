...
| Frontmatter | hidden: a leading `---` … `---`/`...` block is sliced off before parsing, so the page starts at the real content (the editor keeps it editable). An unclosed `---` on line 1 stays a thematic break |
